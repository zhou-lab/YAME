...
grep -q '^\[methprobe fetch\] hg38/probe: 1 of 1 files come from an earlier release of zhou-lab/probe than this build pins:$' r.txt ||
  { echo "the stale-store line is not in the tool's voice"; cat r.txt; exit 1; }
grep -q '^    replace them with: methprobe fetch -y -f hg38/probe$' r.txt ||
  { echo "the repair command is not in the tool's voice"; cat r.txt; exit 1; }
