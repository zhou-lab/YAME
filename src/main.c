...
#include "registry.h"     /* yame's own catalogue: the ONE place it is bound */

/* yame's registry, bound HERE and nowhere in the library. `yame fetch` is
 * fetch over it; a downstream tool's `<tool> fetch` is the same function over
 * its own, which that tool generates itself from the shared catalogue in
...
static const yame_fetch_cfg_t yame_cfg = { YAME_ASSETS, YAME_ASSETS_N, "yame", NULL };
