 ...
 * its own, generated by make_registry.sh --tool=<name>. Library code that
 * needs the running tool's registry (summary -b) gets it through the hook
 * main() registers below -- never by naming a symbol defined in this file,
 * which no downstream links. */
