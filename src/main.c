...
/* `yame fetch` is fetch over yame's registry. A downstream tool's `<tool>
 * fetch` is the same function over its own, generated by make_registry.sh
 * --tool=<name>, so the models its docs name are the ones its binary pins. */
static int main_fetch(int argc, char *argv[]) {
  static const yame_fetch_cfg_t cfg = { YAME_ASSETS, YAME_ASSETS_N, "yame", NULL };
  return yame_fetch_main(&cfg, argc, argv);
}

/* summary -b picks masks through the same browser, over the same registry */
const yame_fetch_cfg_t *yame_own_fetch_cfg(void) {
  static const yame_fetch_cfg_t cfg = { YAME_ASSETS, YAME_ASSETS_N, "yame", NULL };
  return &cfg;
}
