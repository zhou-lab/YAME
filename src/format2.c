...
static uint8_t* compressDataToRLE(cdata_t *c, uint64_t *rle_n) {
  ...
  for (uint64_t i = 0; i < c->n;) {
    ...
    rle = wzrealloc(rle, ((*rle_n) + value_bytes + 2));
...
cdata_t* fmt2_read_raw(char *fname, int verbose) {
  ...
  while (gzFile_read_line(fh, &line) > 0) {
    ...
    if (ret) {  // The key didn't exist before
      ...
      if (keys_n + 1 > keys_m) {
        keys_m <<= 1;
        keys = wzrealloc(keys, keys_m * sizeof(char*));
      }
      keys[keys_n++] = kc;
    } else free(kc);
    if (data_n+1>data_m) {
      data_m <<= 1;
      data = wzrealloc(data, data_m * sizeof(uint64_t));
    }
  ...
  cdata_t *c = wzcalloc(1, sizeof(cdata_t));
  c->compressed = 0;
  c->fmt = '2';
  c->aux = wzcalloc(1, sizeof(f2_aux_t));
  c->n = data_n;  // when uncompressed, c->n is the data length not byte length
  c->s = wzcalloc(1, keys_n_bytes + data_n*sizeof(uint64_t) + 1);

  // Write the keys to the data
  f2_aux_t *aux = (f2_aux_t*) c->aux;
  aux->nk = keys_n;
  aux->data = c->s + keys_n_bytes + 1;
  aux->keys = wzcalloc(keys_n, sizeof(char*));
  uint64_t pos = 0;
  for (uint64_t i = 0; i < keys_n; ++i) {
...
void fmt2_compress(cdata_t *c) {
  ...
  uint8_t *s_out = wzcalloc(keys_nb + rle_n + 1, sizeof(uint8_t));
...
cdata_t fmt2_decompress(const cdata_t c) {
  ...
  inflated.s = wzmalloc(inflated.n);
  if (inflated.s == NULL) {
...
void fmt2_set_aux(cdata_t *c) {
  ...
  f2_aux_t *aux = wzcalloc(1, sizeof(f2_aux_t));
  aux->nk = fmt2_get_keys_n(c);
  aux->keys = (char **)wzmalloc(aux->nk * sizeof(char *));
...
stats_t* summarize1_queryfmt2(
  ...
  if (c_mask->n == 0) {          // no mask
    ...
    f2_aux_t *aux = (f2_aux_t*) c->aux;
    *n_st = aux->nk;
    uint64_t *cnts = wzcalloc(aux->nk, sizeof(uint64_t));
    for (uint64_t i=0; i<c->n; ++i) cnts[f2_get_uint64(c, i)]++;
    st = wzcalloc(aux->nk, sizeof(stats_t));
    ...
    *n_st = aux->nk;
    uint64_t *cnts = wzcalloc(aux->nk, sizeof(uint64_t));
    uint64_t *cnts_q = wzcalloc(aux->nk, sizeof(uint64_t));
    uint64_t n_m = 0;
    for (uint64_t i=0; i<c->n; ++i) {
    ...
    *n_st = aux_m->nk * aux_q->nk;
    ...
    uint64_t *nq = wzcalloc(aux_q->nk, sizeof(uint64_t));
    uint64_t *nm = wzcalloc(aux_m->nk, sizeof(uint64_t));
    for (uint64_t i=0; i<c->n; ++i) {
