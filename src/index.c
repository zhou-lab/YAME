...
index_pair_t *index_pairs(index_t *idx, int *n) {
  ...
  index_pair_t* pairs = (index_pair_t*)wzmalloc(kh_size(idx) * sizeof(index_pair_t));
  *n = 0;
...
int main_index(int argc, char *argv[]) {
  ...
  while ((c0 = getopt(argc, argv, "cs:1:h"))>=0) {
    ...
    case '1': sname_to_append = wzstrdup(optarg); break;
  ...
  if (sname_to_append) {        /* append new sample to existing index */
    ...
    if (snames.n >0) {               /* sample names is given */
      ...
      for (n=0; ; ++n) {
        ...
        sname_v = wzrealloc(sname_v, sizeof(kstring_t)*(n+1));
        addr_v = wzrealloc(addr_v, sizeof(int64_t)*(n+1));
