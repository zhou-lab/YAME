...
int fmt7_next_bed(cdata_t *c) {
  row_reader_t *rdr;
  if (!c->aux) c->aux = wzcalloc(1, sizeof(row_reader_t));
...
cdata_t fmt7_decompress(const cdata_t c) {
  ...
  while (row_reader_next_loc(&rdr, (cdata_t *)&c)) {
    if (chrm != rdr.chrm) {   /* new chromosome encountered */
      ...
      chrms[nchrms] = wzstrdup(chrm);  /* own a copy of the name */
      if (!chrms[nchrms]) {
        fprintf(stderr, "[fmt7_decompress] Out of memory wzstrdup(chrm)\n");
        exit(1);
      }
      nchrms++;
    }
...
stats_t* summarize1_queryfmt7(
  ...
  if (c_mask->n == 0) {
    ...
    *n_st = (uint64_t) nchrms;
    ...
    uint64_t* chrm_cnts = wzcalloc(nchrms, sizeof(uint64_t));
    ...
    for (int ichr = 0; ichr < nchrms; ++ichr) {
      ...
      if (config->section_name) {
        ...
        s->sq = wzstrdup(chrms[ichr] ? chrms[ichr] : "");
      }
    }
