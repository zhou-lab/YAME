...
cdata_t fmt4_decompress(const cdata_t c) {
  ...
  float *s = wzcalloc(m*sizeof(float), 1);

  for(i=0; i< c.n>>2; ++i) {
    if (s0[i] >> 31) {
      l = s0[i]<<1>>1;
      if (n+l+10>m) {m=n+l+10; m<<=1; s = wzrealloc(s, m*sizeof(float));}
      for (j=0; j<l; ++j) s[n++] = -1.0;
    } else {
      if (n+2>m) {m<<=1; s = wzrealloc(s, m*sizeof(float));}
...
cdata_t* fmt4_read_raw(char *fname, int verbose) {
  ...
  char *line = NULL;
  uint64_t n = 0, m=1<<22;
  float *s = wzcalloc(m, sizeof(float));
...
void fmt4_compress(cdata_t *c) {
  ...
  uint64_t n=0, m=1<<20;
  uint32_t *s = wzcalloc(sizeof(uint32_t), m);
